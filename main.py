...
# Машина состояний (FSM) для сохранения шагов пользователя
...
async def start_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Привет, felix! 🤙 Я бот-чекер фандинга.\n\n"
        "Выбери биржу, на которой будем считать накопленный фандинг:",
...
async def process_exchange(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Напиши тикер монеты, которую играешь (например: `BTC`, `ETH`, `SOL` или `ZZZ` если найдешь 😅):",
        parse_mode="Markdown"
    )
...
async def process_period(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏳ *Подключаюсь к бирже, паршу данные...*", parse_mode="Markdown")
    
    try:
        # Идем запрашивать данные по API
        ...
        if total_funding is None:
            await callback.message.edit_text(
                ...
                f"Возможно, пара не существует или биржа вернула ошибку.",
                parse_mode="Markdown",
            ...
            await callback.message.edit_text(
                ...
                f"💡 *Справка: если цифра положительная — всё это время лонгисты платили шортистам. Если отрицательная — шортисты башляли лонгистам.*",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="Считать заново 🔄", callback_data="restart")]
                ])
        ...
        logging.error(f"Error fetching funding: {e}")
        await callback.message.edit_text("❌ Произошла ошибка при вычислении. Попробуй позже.")

@dp.callback_query(F.data == "restart")
async def restart_handler(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.edit_text(
        "Выбери биржу для нового расчета:",
        reply_markup=get_exchanges_kb()
    )


# --- ФУНКЦИИ ЗАПРОСОВ К API (AIOHTTP) ---
...
async def fetch_funding(exchange: str, coin: str, days: int) -> float | None:
    ...
    async with aiohttp.ClientSession() as session:
        if exchange == "mexc":
            return await fetch_mexc(session, coin, limit_ms)
        elif exchange == "gate":
            return await fetch_gate(session, coin, limit_ms)
        elif exchange == "bitget":
            return await fetch_bitget(session, coin, limit_ms)
    return None

async def fetch_mexc(session, coin, limit_ms):
    symbol = f"{coin}_USDT"
    url = f"https://contract.mexc.com/api/v1/contract/funding_rate/history?symbol={symbol}&page_num=1&page_size=1000"
    async with session.get(url) as resp:
        ...
        total = 0.0
        for item in rates:
            if item["settleTime"] >= limit_ms:
                total += float(item["fundingRate"])
            else:
                break
        return total * 100  # Переводим в проценты

async def fetch_gate(session, coin, limit_ms):
    symbol = f"{coin}_USDT"
    url = f"https://api.gateio.ws/api/v4/futures/usdt/funding_rate?contract={symbol}&limit=1000"
    async with session.get(url) as resp:
        if resp.status != 200: return None
        rates = await resp.json()
        
        total = 0.0
        limit_sec = limit_ms / 1000 # У gate.io таймстемп в секундах
        for item in rates:
            if item["t"] >= limit_sec:
                total += float(item["r"])
            else:
...
async def fetch_bitget(session, coin, limit_ms):
    symbol = f"{coin}USDT"
    # Bitget отдает историю по API V2, запрашиваем макс. лимит
    url = f"https://api.bitget.com/api/v2/mix/market/history-fund-rate?symbol={symbol}&productType=USDT-FUTURES&pageSize=100"
    async with session.get(url) as resp:
        if resp.status != 200: return None
        data = await resp.json()
        ...
        total = 0.0
        for item in rates:
            if int(item["fundingTime"]) >= limit_ms:
                total += float(item["fundingRate"])
            else:
                break
        return total * 100
